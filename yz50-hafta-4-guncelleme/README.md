...
`hafta-4/YZ50_Week4_MLP_Language_Model.ipynb` — Bengio 2003 tarzı çok katmanlı algılayıcı (MLP) ile karakter düzeyinde dil modeli. Hafta 3'ün bigram modelinin aksine 3 harflik bağlama (`block_size = 3`) bakılıyor ve karakterler öğrenilebilir embedding'lerle temsil ediliyor (Görev 1–7):
...
6. **BatchNorm** — gizli katmandan sonra, `tanh`'tan önce BatchNorm: öğrenilebilir `bngain`/`bnbias` (BN ortalamayı çıkardığı için `b1` kaldırıldı), eğitimde batch istatistiği, tahminde `running` mean/std buffer'ları (momentum 0.001). Üç model aynı eğitimle kıyaslanıyor: Kaiming (BN yok), Kaiming + BN ve W1 ölçeksiz + BN — BN'nin kötü init'e dayanıklılığı. Ayrıca running istatistiklerin, eğitim sonunda tüm train seti üzerinden ölçülen kalibre mean/std ile sağlaması.
7. **Türkçe isimler** — `turkce_isim_clean.txt` (13.865 isim, ç ğ ı ö ş ü dahil 30 karakterlik alfabe) ile aynı pipeline. Dosya alfabetik sıralı olduğu için bölmeden önce karıştırılıyor. MLP dev 2.151, bigram dev 2.491.
...
## Hafta 5 — Elle Backpropagation (makemore part 4)

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/bluegreensun/yz50/blob/main/hafta-5/YZ50_Week5_Manual_Backpropagation.ipynb)

`hafta-5/YZ50_Week5_Manual_Backpropagation.ipynb` — Hafta 4'teki MLP'nin (embedding + BatchNorm + `tanh`, `n_embd = 10`, `n_hidden = 64`, 4.137 parametre) gradyanlarını `loss.backward()` kullanmadan, tek tek elle yazma (Görev 1–2):

1. **Atomik forward pass** — 32'lik tek bir minibatch üzerinde forward pass'in en küçük adımlara bölünmesi: BatchNorm'un parçaları (`bnmeani`, `bndiff`, `bnvar` — Bessel düzeltmesiyle `n-1`, `bnvar_inv`, `bnraw`) ve cross entropy'nin parçaları (`logit_maxes`, `norm_logits`, `counts`, `probs`, `logprobs`). Elle hesaplanan loss (3.3206) `F.cross_entropy` ile aynı; ara değişkenlerde `retain_grad()` ile PyTorch gradyanları referans olarak saklanıyor.
2. **Elle backward pass** — loss'tan embedding tablosu `C`'ye kadar 26 gradyanın zincir kuralıyla yazılması. Üç kural: gradyanın shape'i değişkenin shape'iyle aynıdır; forward'da broadcast olan eksende backward'da `sum` alınır (ve tersi); grafikte iki dala giden değişkenin gradyanları toplanır (`+=`). `cmp` fonksiyonuyla PyTorch'a karşı doğrulama: 26 gradyanın 11'i birebir, 15'i `allclose` ile eşleşiyor (en büyük fark ~6e-9, floating point), hatalı yok.

Veri dosyası (`names.txt`) klasörün içinde; Colab'da açınca sol panelden yüklemek gerekir. Yerelde:

```bash
cd hafta-5
pip install -r requirements.txt
jupyter notebook YZ50_Week5_Manual_Backpropagation.ipynb
```
